...
sub check_network
{
	...
	foreach my $node_name (sort keys %$network_ref)
	{
		...
		my $node = $$network_ref{$node_name};
		my $val = $node->check_pedigree_relationships($network_ref,$phase); ## I am running generation check for every node, running it once should be sufficient.
		if($val > 0){return $val;}
	...
	{
		print "Networt $network_ref has $num_generations generations. Exceeds MAX_GENERATIONS: $MAX_GENERATIONS\n";
		print $LOG "Networt $network_ref has $num_generations generations. Exceeds MAX_GENERATIONS: $MAX_GENERATIONS\n";
		return 101;
	}
